...
def init_db():
    ...
    # Enforce additional uniqueness and deduplication rules via indexes
    # Unique phone per user (allowing many NULLs)
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS users_phone_unique ON users (phone) WHERE phone IS NOT NULL;
    """)

    # Prevent duplicate implements for same owner with same name
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS implements_owner_name_unique ON implements (owner_id, name);
    """)

    # Prevent two active operations running on the same tractor
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS operations_active_unique ON operations (tractor_id) WHERE status = 'active';
    """)

    # Prevent duplicate telemetry points (operation + tractor + timestamp)
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS telemetry_unique ON telemetry (operation_id, tractor_id, timestamp);
    """)

    # Prevent duplicate fuel logs at the exact same timestamp for a tractor
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS fuel_logs_unique ON fuel_logs (tractor_id, timestamp);
    """)

    # Prevent duplicate alerts for the same tractor/operation/type/timestamp
    cursor.execute("""
